class StatisticsController < ApplicationController
  def index
    ...
    activity = {total: Activity.all.size, total_active: Activity.active(Time.zone.now).size, active_activities_locations: Activity.select('latitude,longitude').active(Time.zone.now).privacy_location(true), active_activities_locations_local: Activity.select('latitude,longitude').active(Time.zone.now).privacy_location(false)}
