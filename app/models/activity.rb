class Activity < ActiveRecord::Base
  ...
  def send_edited_notification
    ...
    self.attendees.each do |user|
      user.devices.each do |device| 
         ...
         notifications << APNS::Notification.new(device.token,{:alert => "The activity \"#{self.title}\" that you are attending has been updated!",:badge => device.badge,:sound => 'default',:other => {info: {id: self.activity.id, type: 'activity'}}})
      end
    end
  ...
  def send_created_notification
    ...
    User.in_range(self.latitude,self.longitude).each do |user|
      user.devices.each do |device| 
         device.update_attribute(:badge,device.badge.to_i + 1)
         notifications << APNS::Notification.new(device.token,{:alert => "The activity \"#{self.title}\" has been created on #{self.start.strftime('%b %d')} at #{self.start.strftime('%H:%M')}",:badge => device.badge,:sound => 'default',:other => {info: {id: self.activity.id, type: 'activity'}}})
      end
    end
    APNS.send_notifications(notifications) unless notifications.empty?
  end
  ...
  def send_deleted_notificaiton
    ...
    self.attendees.each do |user|
      user.devices.each do |device| 
         device.update_attribute(:badge,device.badge.to_i + 1)
         notifications << APNS::Notification.new(device.token,{:alert => "The activity \"#{self.title}\" that you would attend has been deleted!",:badge => device.badge,:sound => 'default',:other => {info: {id: self.activity.id, type: 'activity'}}})
      end
    end
    APNS.send_notifications(notifications) unless notifications.empty?
  end
